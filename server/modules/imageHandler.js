...
const uploadPostWithText = async (req, res) => {
  console.log(`Hit uploadPostWithText!`);
...
const generateSignedUrls = async (res, rows) => {
  console.log(`Hit generateSignedUrls!`);
...
function generateSignedUrl(key) {
  console.log(`Hit generateSignedUrl!`);
...
function uploadToSQL(req, media_key, res) {
  console.log(`Hit uploadToS3! LINE 106`);
  return new Promise(resolve => {
    const queryText = `INSERT INTO "images" 
                        ("media_key")
                        VALUES
                        ($1)`;
...
function uploadToSQLWithText(req, media_key, res) {
  console.log(`Hit uploadToSQLWithText!`);
  return new Promise(resolve => {
    ...
    pool.query(queryText, [media_key, title, content])
      .then((result) => {
        console.log(`Hit uploadToSQLWithText.then !`);
        verbose && console.log('back from db with:', result);
        res.sendStatus(200);
      })
      .catch((error) => {
